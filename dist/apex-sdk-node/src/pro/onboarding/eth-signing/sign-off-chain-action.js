...
class SignOffChainAction extends signer_1.Signer {
    ...
    sign(signer, signingMethod, message, env) {
            ...
            switch (signingMethod) {
                ...
                case main_1.SigningMethod.Compatibility: {
                    ...
                    const rawSignature = walletAccount
                        ...
                        : yield this.web3.eth.sign(hash, signer);
                ...
                case main_1.SigningMethod.TypedData:
                    ...
                    if (walletAccount.privateKey) {
                        ...
                        const rawSignature = yield wallet._signTypedData(this.getDomainData(), { [this.domain]: this.actionStruct }, message);
                ...
                case main_1.SigningMethod.CoinbaseWallet: {
                    ...
                    const signature = yield this.ethSignTypedDataInternal(signer, data, signingMethod);
                    return signature;
                }
    ...
    getPersonalSignMessage(message) {
        ...
        const json = JSON.stringify(Object.assign(Object.assign({}, lodash_1.default.pick(this.getDomainData(), PERSONAL_SIGN_DOMAIN_PARAMS)), lodash_1.default.pick(message, lodash_1.default.keys(message).sort())), null, 2);
        return json
            .replace('{\n', '')
            .replace('\n}', '')
            .replace(/"/g, '')
