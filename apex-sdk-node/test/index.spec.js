...
import { OrderSide, OrderType } from '../src/omni/interface';
import { Trace } from '../src/omni/tool/Tool';
import { ApexClient, ApexClientV2, generateRandomClientId, OMNI_QA, QA } from '../src';
...
describe('Omni&Pro Api Example', () => {
    ...
    before(async () => {
        ...
        const seed = omniUser.seed;
        await OMNI_apexClient.init(OMNI_apiKeyCredentials, seed);
    });
    it('GET Accounts', async () => {
        // pro
        const account = await apexClient.privateApi.getAccountV2(apexClient.clientConfig.accountId, apexClient.account.ethereumAddress);
        Trace.print(account);
        // omni
        const OMNI_account = await OMNI_apexClient.privateApi.getAccount(OMNI_apexClient.clientConfig.accountId, OMNI_apexClient.account.ethereumAddress);
