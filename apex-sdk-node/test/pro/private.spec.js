...
import { ApexClient, generateRandomClientId, OrderSide, OrderType, QA, Trace, } from '../../src/pro';
...
describe('Private Api Example', () => {
    ...
    it('POST Creating Orders', async () => {
        ...
        const result = await apexClient.privateApi.createOrder(apiOrder.clientOrderId, apiOrder.positionId, apiOrder.symbol, apiOrder.side, apiOrder.type, apiOrder.size, apiOrder.price, apiOrder.limitFee, apiOrder.timeInForce, apiOrder.triggerPrice, apiOrder.trailingPercent, apiOrder.reduceOnly);
        Trace.print(result);
    });
    ...
    it('GET Funding Rate', async () => {
        const { fundingValues, totalSize } = await apexClient.privateApi.fundingRate(currentPerpetual);
