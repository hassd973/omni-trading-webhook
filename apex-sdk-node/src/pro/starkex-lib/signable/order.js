// apex-sdk-node/src/pro/starkex-lib/signable/order.js
const BN = require('bn.js');
const { addOrderExpirationBufferHours, isoTimestampToEpochHours } = require('../helpers');
const { getPedersenHash } = require('../crypto/proxies');
const { decToBn, hexToBn, intToBn } = require('../lib/util');
const { ORDER_FIELD_BIT_LENGTHS } = require('./constants');
...
function assetToBaseQuantumNumber(symbol, humanAmount, multiplier) {
  // Convert humanAmount to quanta (e.g., 1.5 to 1500000)
  const amount = parseFloat(humanAmount);
  const mult = parseInt(multiplier.replace(/^1e/, '1'));
  return Math.floor(amount * mult).toString();
}

class SignableOrder extends StarkSignable {
  ...
  static fromOrderWithNonce(order, networkId) {
    ...
    const quantumsAmountCollateral = assetToBaseQuantumNumber(
      order.symbol,
      'quoteAmount' in order ? order.quoteAmount : '0',
      '1e6'
    );
    ...
    const expirationEpochHours = addOrderExpirationBufferHours(
      isoTimestampToEpochHours(order.expirationIsoTimestamp).toString()
    );
  ...
  async calculateHash() {
    ...
    const expirationEpochHours = intToBn(parseInt(message.expirationEpochHours));
